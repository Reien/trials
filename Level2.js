...
Project.Level2.prototype = {
	create: function(game) {
		// Game BGM
		music = this.sound.play("second");

		// Game Physics, Bounds and Background
		this.physics.startSystem(Phaser.Physics.ARCADE);
		this.stage.backgroundColor = "#636363";
		this.add.tileSprite(0, 0, 640, 4000, "background2");
		...
		// Generating Ledges
		ledge = platforms.create(0,this.world.height - 104, "ledge");
		...
		for (var i=0; i < 50; i++) {
			...
			ledge = platforms.create(ledW, ledH, "ledge");
			...
			j = Math.random() * 2;
			if (j < 1) {
		    	coins = items.create(ledW + 80, ledH + 80, "gem");
        ...
        // Generating Walls
	    ledge = platforms.create(0,0, 'wallL');
		ledge.body.immovable = true;
		
		ledge = platforms.create(600,0, 'wallR');
		ledge.body.immovable = true;
		...
		for (var i=0; i<150; i++){
			...
			hazzard = spike.create(55,(20*rand)*i, 'spikes');
			hazzard.body.immovable = true;
		}
			
		for (var i=0; i<100; i++){
			var rand = Math.random() * 3;
			hazzard = spike.create(595,(30*rand)*i, 'spikes2');		
			hazzard.body.immovable = true;
		}
		
		// Player Settings
		player = this.add.sprite(300, this.world.height - 200, "medieval");
