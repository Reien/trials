...
Project.Level3.prototype = {
	create: function() {
		// Game BGM
		music = this.sound.play("third");

		// Game Physics, Bounds and Background
		this.physics.startSystem(Phaser.Physics.ARCADE);
		this.stage.backgroundColor = "#636363";
		this.add.tileSprite(0, 0, 640, 4000, "background3");
		...
		// Generating Ledges
		ledge = platforms.create(0,this.world.height - 104, "ground");
		...
		for (var i=0; i < 50; i++) {
			...
			ledge = platforms.create(ledW, ledH, "ground");
		...
		for (var i=0; i<150; i++){
			...
			hazzard = spike.create(20,(20*rand)*i, 'spikes');
			hazzard.body.immovable = true;
		}
			
		for (var i=0; i<100; i++){
			var rand = Math.random() * 3;
			hazzard = spike.create(540,(30*rand)*i, 'spikes2');		
			hazzard.body.immovable = true;
		}
		
		// Player Settings
		player = this.add.sprite(300, this.world.height - 200, "future");
